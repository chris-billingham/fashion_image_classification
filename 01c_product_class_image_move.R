...
if(!dir.exists("modelling_images/product_classification/train")){dir.create("modelling_images/product_classification/train")}
if(!dir.exists("modelling_images/product_classification/valid")){dir.create("modelling_images/product_classification/valid")}
if(!dir.exists("modelling_images/product_classification/test")){dir.create("modelling_images/product_classification/test")}

# iterate through all combinations to build directory structure
directory_structure <- paste0("modelling_images/product_classification/", grid$Var1, "/", grid$Var2) %>%
...
images_train <- images_train %>%
  mutate(new_path = paste0("modelling_images/product_classification/train/", category, "/", value))
...
images_valid <- images_valid %>%
  mutate(new_path = paste0("modelling_images/product_classification/valid/", category, "/", value))
images_test <- images_test %>%
  mutate(new_path = paste0("modelling_images/product_classification/test/", category, "/", value))
